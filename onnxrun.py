...
class BaseModel:
    ...
    def __init__(self,
        ...
        # self.model.load_state_dict(torch.load((model_path),map_location=torch.device('cpu'))["params"])
    ...
    def __call__(self, img: np.ndarray) -> np.ndarray:
        ...
        st = time.time()
        out = self.sess.run([output], {input: img})[0]
        # out = self.model(torch.from_numpy(img))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str,default='optimized_model.onnx', help='specify onnx model path')
    parser.add_argument('--input_path', type=str,default='datasets/C0007_multiscale_sub/00000T0_s001.jpg', help='specify input model path')
    parser.add_argument('--output_path', type=str,default='inputs/anh21.jpg' , help='specify output model path')
    parser.add_argument('--output_scale', type=int, default=2, help='specify the output scale, defaults to 4')
    parser.add_argument('--num_threads', type=int, default=-1, help='specify num threads of onnx')
    args = parser.parse_args()

    model = RealESRGAN(args.model_path, intra_op_num_threads=args.num_threads, pre_pad=0)
